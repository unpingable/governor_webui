...
The assistant says something. The UI detects "definition/citation"-shaped content.
Detections show up as **chips** and collect in a **Pending** drawer.
Nothing auto-promotes. **Accept** writes to the canonical store *with a receipt*.
...
Receipts come from the governor daemon, not the UI.

<!-- Screenshots generated by: npm run screenshots (see tools/screenshots/) -->
![Research mode — chat with DOI refs + governance sidebar](docs/img/01_research_home.png)
![Dashboard — dark mode](docs/img/04_dashboard.png)
